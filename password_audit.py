...
def build_hash_dict(common_passwords):
    ...
    Value = Wordlist password used internally for match detection only (never written to the audit report)

    Using a dictionary (instead of a set) allows O(1) lookup speed while
    preserving the matched plaintext internally for detection logic, without
    storing plaintext in the final audit report.
    """
...
def get_compromised_password(user_hash, common_hash_dict):
    """
    Return the plaintext password if the hash exists in the dictionary (used
    internally for match detection only — not written to the audit report).
...
def check_passwords(user_data, common_hash_dict):
    ...
    Returns a list of result dictionaries. Plaintext passwords are NEVER stored — only cracked status and risk category.
    """
    results = []
    vulnerable_count = 0
    processed_count = 0
    ...
    for user in user_data:
        ...
        found = matched_password is not None
        if found:
            vulnerable_count += 1
        ...
        results.append({
            "username": username,
            "hash": stored_hash,
            "password_cracked": found,
            "risk": "weak_dictionary_password_detected" if found else "not_found_in_wordlist"
        })
...
def main():
    ...
    # Build hash dictionary — O(1) lookups for match detection
    ...
    # Interactive mode — optional, run with --interactive flag
    import sys
