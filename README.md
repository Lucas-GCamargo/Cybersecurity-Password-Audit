...
build_hash_dict()           pre-computes a hash→plaintext dictionary
        ↓
get_compromised_password()  executes the O(1) hash lookup per user
        ↓
check_passwords()           loops through users — matched = vulnerable
        ↓
save_results()              writes findings to output.json
        ↓
interactive_mode()          real-time SHA-256 hashing for manual testing
```
...
## ⚡ Algorithmic Design: O(1) Lookup with Plaintext Recovery
...
**This project implements the production-grade approach** — the wordlist is pre-processed into a Python dictionary where each key is a SHA-256 hash and each value is the original plaintext password:

```python
# build_hash_dict() — pre-computes the hash→plaintext dictionary once
common_hash_dict = {hash_password(p): p for p in common_passwords}

# get_compromised_password() — O(1) lookup, called once per user inside check_passwords()
matched_password = common_hash_dict.get(stored_hash)
```

This achieves two goals simultaneously: O(1) lookup speed **and** the ability to recover the original plaintext password for inclusion in the audit report — something a plain `set` cannot do.
...
============================================================
  Cybersecurity Password Audit Tool
  Author: Lucas Camargo | TAFE Cybersecurity 2026
============================================================
...
Type any string to see its SHA-256 hash. Type 'exit' to quit.

Enter a string to hash (or 'exit'): 
```
...
Accounts where `"password_found": true` have hashes matching a known common password and should be immediately flagged for a mandatory password reset.
...
Switching from a linear search to a pre-computed hash dictionary reduced the lookup complexity from O(n × m) to O(1), while also solving the problem of recovering plaintext passwords for the audit report. A `set` would give speed but lose the plaintext — a `dict` achieves both.
...
This project was completed as part of the Cybersecurity program at TAFE. I am developing hands-on skills in Python scripting, ethical hacking fundamentals, and defensive security tooling.
...
This tool was created for educational purposes as part of a TAFE Cybersecurity assessment. The author accepts no responsibility for misuse.
